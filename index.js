...
const mostrarProductosTienda = (productosTienda) => {
  ...
  if (productosTienda.length === 0) {
    ...
    mensaje.classList.add("mensajeBusqueda");
    mensaje.textContent =
      "No se encontraron resultados para tu búsqueda. Aquí tienes algunas recomendaciones:";
    mensaje.style.textAlign = "center";
    mensajeDiv.appendChild(mensaje);
    sectionCardsZapas.appendChild(mensajeDiv);

    const zapatillasAleatorias = listaZapatillas
      .sort(() => 0.5 - Math.random())
      .slice(0, 3);

    zapatillasAleatorias.forEach((producto) => {
      const cardZapas = document.createElement("div");
      cardZapas.classList.add("card-zapas");
      cardZapas.style.borderRadius = "10px";
      cardZapas.style.marginTop = "80px";
  ...
  productosTienda.forEach((producto) => {
    ...
    cardZapas.classList.add("card-zapas");
    cardZapas.style.borderRadius = "10px";
    cardZapas.style.marginTop = "35px";
